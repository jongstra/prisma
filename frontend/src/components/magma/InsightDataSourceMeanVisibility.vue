...
function getDataSourceVisibility() {
  const useCases = magma.L3UseCases(store.domain);

  // Object to store the use case visiblility values and keep a use case count, for each dataSource.
  const stats = {};
  ...
  useCases.forEach(useCase => {
    ...
    stats[useCase.dataSource].sum += Number(useCase.visibility);
...
<template>
  <div class="item-visualization">
    <div class="title">
      Data Sources - Mean Visibility
      <span v-if="Object.keys(getDataSourceVisibility()).length >= 15"> (Top 15 Worst)</span>
    </div>
    <hr>

    <div v-for="[key, val] in Object.entries(getDataSourceVisibility()).slice(0, 15)" class="item-row">
